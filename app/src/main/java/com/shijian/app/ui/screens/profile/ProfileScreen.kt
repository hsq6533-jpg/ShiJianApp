...
private fun WorkTimeSheet(
    ...
    val sheetState = rememberModalBottomSheetState(skipPartiallyExpanded = true)
    var startStr by remember { mutableStateOf(formatMinutes(currentStart)) }
    var endStr by remember { mutableStateOf(formatMinutes(currentEnd)) }
    ...
    ModalBottomSheet(onDismissRequest = onDismiss, sheetState = sheetState) {
        Column(modifier = Modifier.padding(horizontal = 20.dp).padding(bottom = 28.dp)) {
            ...
            SjPrimaryButtonSmall(
                text = "完成",
                onClick = {
                    val startMin = parseTime(startStr)
                    val endMin = parseTime(endStr)
                    if (startMin == null || endMin == null) {
                        errorMsg = "时间格式错误，请输入正确的时间（如 09:00）"
                        return@SjPrimaryButtonSmall
                    }
                    if (startMin >= endMin) {
                        errorMsg = "上班时间必须早于下班时间"
                        return@SjPrimaryButtonSmall
                    }
